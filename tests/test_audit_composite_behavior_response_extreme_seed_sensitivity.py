from scripts.audit_composite_behavior_response_extreme_seed_sensitivity import build_report, summarize_case


def test_response_extreme_summary_requires_candidate_in_all_seeds(monkeypatch) -> None:
    rows = [
        {
    ...
    monkeypatch.setattr(
        "scripts.audit_composite_behavior_response_extreme_seed_sensitivity.load_case_row",
    ...
    assert not summary["robust_response_extreme_seed_candidate"]


def test_build_report_marks_gpu_ready_for_robust_local_candidate(monkeypatch) -> None:
    row = {
        "target_mode": "t",
        "family": "f",
        "holdout": "h",
        ...
        "robust_response_extreme_seed_candidate": True,
        "seed_rows": [],
    }

    monkeypatch.setattr(
        "scripts.audit_composite_behavior_response_extreme_seed_sensitivity.summarize_case",
        lambda **kwargs: row,
    )
    ...
    assert report["summary"]["decision"] == "response_extreme_seed_candidate"
    assert report["summary"]["max_candidate_seed_fraction"] == 1.0
    assert report["summary"]["gpu_training_ready"]
