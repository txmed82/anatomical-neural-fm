from scripts.audit_neutral_prior_low_contrast_choice_seed_sensitivity import summarize_case


def test_neutral_prior_low_contrast_seed_summary_rejects_seed_fragile_candidate(monkeypatch) -> None:
    rows = {
    ...
    monkeypatch.setattr(
        "scripts.audit_neutral_prior_low_contrast_choice_seed_sensitivity.load_case_row",
        fake_load_case_row,
    )
    ...
    assert not summary["robust_neutral_prior_low_contrast_choice_seed_candidate"]


def test_neutral_prior_low_contrast_seed_summary_accepts_all_candidate_seeds(monkeypatch) -> None:
    def fake_load_case_row(_seed: int, _case: dict) -> dict:
        return {
            "decision": "candidate",
    ...
    assert summary["robust_neutral_prior_low_contrast_choice_seed_candidate"]
