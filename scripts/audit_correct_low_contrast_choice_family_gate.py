"""Audit correct low-contrast choice targets under the shared-family gate."""
...
def target_name_for_threshold(threshold: float) -> str:
    return f"correct_low_contrast_choice_le_{threshold:g}"


def per_recording_correct_low_contrast_choice_labels(rec, *, max_contrast: float) -> np.ndarray:
...
def build_correct_low_contrast_choice_trial_samples(
    ...
    *,
    ...
    for rid in rids:
        rec = recs[rid]
        labels = per_recording_correct_low_contrast_choice_labels(rec, max_contrast=max_contrast)
        stim_on = _finite_array(rec.trials.stim_on_times)
...
def precompute_correct_threshold(**kwargs) -> dict:
    # Reuse the feature/control implementation by temporarily swapping the trial builder.
    ...
    low_contrast.build_low_contrast_choice_trial_samples = build_correct_low_contrast_choice_trial_samples
...
def summarize(rows: list[dict], balances: dict[str, dict]) -> dict:
    ...
    base["decision"] = (
        "correct_low_contrast_choice_family_candidate"
        if base["n_candidates"]
        else "no_correct_low_contrast_choice_family_candidate"
    )
    base["next_action"] = (
        "Validate correct low-contrast choice candidates across shuffle seeds before GPU training."
        if base["n_candidates"]
        else "Do not train: correct low-contrast choice targets do not pass the strict local gate."
    )
    return base
...
def render_markdown(report: dict) -> str:
    ...
    text = text.replace("# Low-Contrast Choice Family Gate", "# Correct Low-Contrast Choice Family Gate", 1)
    text = text.replace(
        "restrict trials by absolute stimulus contrast and classify left-vs-right choice",
        "restrict trials by absolute stimulus contrast, keep correct trials, and classify left-vs-right choice",
        1,
    )
    return text
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-json",
        type=Path,
        default=REPO_ROOT / "docs/correct_low_contrast_choice_family_gate.json",
    )
    parser.add_argument(
        "--out-md",
        type=Path,
        default=REPO_ROOT / "docs/correct_low_contrast_choice_family_gate.md",
    )
...
def main() -> int:
    ...
    for threshold in args.max_contrast:
        payload = precompute_correct_threshold(
            ds=ds,
            vocab=vocab,
