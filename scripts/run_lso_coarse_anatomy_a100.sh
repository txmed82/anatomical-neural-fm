#!/usr/bin/env bash
# Coarse-anatomy leave-subject-out confirmation.
#
# Uses Allen parent acronyms for region embeddings and shared-region masking.
set -euo pipefail
...
for holdout in $SUBJECTS; do
  ...
  for seed in $SEEDS; do
    for arm in $ARMS; do
      ...
      rm -rf "$out"
      echo ""
      echo "=== holdout=$holdout arm=$arm seed=$seed region_granularity=$REGION_GRANULARITY ==="
