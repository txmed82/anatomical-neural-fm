"""Aggregate shuffle-seed sensitivity for the low-contrast choice candidate."""
...
def seed_path(seed: int) -> str:
    if seed == 0:
        return "docs/low_contrast_choice_family_gate_projected_hdf5.json"
    return f"docs/low_contrast_choice_family_gate_projected_hdf5_seed_{seed}.json"
...
def summarize_case(
    *,
    ...
    return {
        ...
        "robust_low_contrast_choice_seed_candidate": robust,
        "seed_rows": seed_rows,
    }
...
def build_report(*, seeds: list[int] | None = None, min_positive_seed_fraction: float = 0.8) -> dict:
    ...
    robust = [row] if row["robust_low_contrast_choice_seed_candidate"] else []
    return {
        "case": DEFAULT_CASE,
        "thresholds": {
            "seeds": seeds,
        ...
        "summary": {
            "decision": (
                "low_contrast_choice_seed_candidate"
                if robust else
                "no_low_contrast_choice_seed_candidate"
            ),
            "n_cases": 1,
            "n_robust_low_contrast_choice_seed_candidates": len(robust),
            "max_positive_shuffle_delta_fraction": row["positive_shuffle_delta_fraction"],
            "gpu_training_ready": False,
            "next_action": (
                "Do not train: the low-contrast choice candidate does not remain a strict "
                "candidate across shuffle seeds."
                if not robust
                else "Run independent prospective-manifest validation before GPU training."
            ),
        },
        "rows": [row],
        "robust_low_contrast_choice_seed_candidates": robust,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-json", type=Path, default=REPO_ROOT / "docs/low_contrast_choice_seed_sensitivity.json")
    parser.add_argument("--out-md", type=Path, default=REPO_ROOT / "docs/low_contrast_choice_seed_sensitivity.md")
