"""Aggregate shuffle-seed sensitivity for subject-stable near misses."""
...
CASES = [
    {
        ...
        "holdout": "KS014",
        "seed0_path": "docs/derived_target_family_gate.json",
        "seed_path_template": "docs/subject_stable_shuffle_seed_derived_{seed}.json",
    },
    {
        "source": "wheel",
        "target_mode": "wheel_active",
        "family": "broad_named_anatomy",
        "holdout": "KS014",
        "seed0_path": "docs/wheel_target_family_gate.json",
        "seed_path_template": "docs/subject_stable_shuffle_seed_wheel_{seed}.json",
    },
    {
        "source": "wheel",
        "target_mode": "wheel_displacement",
        "family": "broad_named_anatomy",
        "holdout": "KS014",
        "seed0_path": "docs/wheel_target_family_gate.json",
        "seed_path_template": "docs/subject_stable_shuffle_seed_wheel_{seed}.json",
    ...
    {
        "source": "reaction",
        "target_mode": "post_stim_speedup",
        "family": "broad_named_anatomy",
        "holdout": "KS014",
        "seed0_path": "docs/reaction_dynamics_target_family_gate.json",
        "seed_path_template": "docs/subject_stable_shuffle_seed_reaction_{seed}.json",
    },
    {
        "source": "reaction",
...
def summarize_case(case: dict, seeds: list[int], *, min_positive_seed_fraction: float) -> dict:
    ...
    mean_delta = sum(row["centered_delta_vs_shuffle"] for row in seed_rows) / len(seed_rows)
    ...
    return {
        ...
        "mean_centered_delta_vs_shuffle": mean_delta,
...
def build_report(
    ...
    *,
    ...
    decision = (
        "subject_stable_shuffle_seed_candidate"
        if robust else
        "no_subject_stable_shuffle_seed_candidate"
    )
    return {
        "thresholds": {
            "seeds": seeds,
        ...
        "summary": {
            ...
            "max_positive_shuffle_delta_fraction": max(
                (row["positive_shuffle_delta_fraction"] for row in rows), default=0.0
            ),
            "gpu_training_ready": False,
            "next_action": (
                "Do not train: subject-stable near misses do not robustly beat shuffled anatomy across seeds."
                if not robust
                else "Run a pre-registered held-out validation gate before any GPU training."
            ),
        },
        "rows": rows,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-json",
        type=Path,
        default=REPO_ROOT / "docs/subject_stable_shuffle_seed_sensitivity.json",
    )
    parser.add_argument(
        "--out-md",
        type=Path,
        default=REPO_ROOT / "docs/subject_stable_shuffle_seed_sensitivity.md",
    )
