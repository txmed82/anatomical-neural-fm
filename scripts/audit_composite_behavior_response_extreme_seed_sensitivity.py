"""Aggregate seed sensitivity for post-error response-extreme composite targets."""
...
def seed_path(seed: int) -> str:
    if seed == 0:
        return "docs/composite_behavior_response_extreme_family_gate_projected_hdf5.json"
    return f"docs/composite_behavior_response_extreme_family_gate_projected_hdf5_seed_{seed}.json"
...
def summarize_case(
    *,
    ...
    return {
        ...
        "robust_response_extreme_seed_candidate": robust,
        "seed_rows": seed_rows,
    }
...
def build_report(
    *,
    ...
    robust = [row for row in rows if row["robust_response_extreme_seed_candidate"]]
    return {
        "cases": list(cases),
        "thresholds": {
            "seeds": seeds,
        ...
        "summary": {
            "decision": (
                "response_extreme_seed_candidate"
                if robust else
                "no_response_extreme_seed_candidate"
            ),
            "n_cases": len(rows),
            "n_robust_response_extreme_seed_candidates": len(robust),
            ...
            "gpu_training_ready": bool(robust),
            "next_action": (
                "Run a bounded A100 pilot for the robust response-extreme candidates under the existing cost cap."
                if robust
                else "Do not train: no response-extreme row remains strict across shuffle seeds."
            ),
        },
        "rows": rows,
        "robust_response_extreme_seed_candidates": robust,
    }
...
def render_markdown(report: dict) -> str:
    ...
    lines = [
        ...
        "",
        (
            "Reruns the projected post-error response-extreme broad-anatomy candidates "
            "across within-recording shuffle seeds."
        ),
        "",
        f"- cases: `{summary['n_cases']}`",
        f"- robust response-extreme seed candidates: `{summary['n_robust_response_extreme_seed_candidates']}`",
    ...
    for row in report["robust_response_extreme_seed_candidates"]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-md",
        type=Path,
        default=REPO_ROOT / "docs/composite_behavior_response_extreme_seed_sensitivity.md",
    )
