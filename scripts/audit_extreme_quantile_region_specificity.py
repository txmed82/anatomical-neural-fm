"""Scan parent-region specificity for the extreme response-latency candidate."""
...
from audit_extreme_quantile_target_family_gate import (  # noqa: E402
    build_extreme_trial_samples,
)
...
def evaluate_region_row(
    *,
    ...
    true = evaluate_feature_set(
        name=f"{region}_true",
        train_x=train_region_true[:, [region_index]],
        train_y=train_y,
        eval_x=eval_region_true[:, [region_index]],
        eval_y=eval_y,
    ...
    shuffle = evaluate_feature_set(
        ...
        train_x=train_region_shuffle[:, [region_index]],
        train_y=train_y,
        eval_x=eval_region_shuffle[:, [region_index]],
        eval_y=eval_y,
        eval_recording_ids=eval_recordings,
        l2=l2,
    )
    ...
    row = {
        ...
        "region": region,
        "holdout": holdout,
        "train_nonzero_fraction": float(np.mean(train_region_true[:, region_index] > 0.0)),
        "eval_nonzero_fraction": float(np.mean(eval_region_true[:, region_index] > 0.0)),
...
def summarize_rows(rows: list[dict]) -> dict:
    ...
    by_holdout = {}
    for holdout in sorted({row["holdout"] for row in rows}):
        holdout_rows = [row for row in rows if row["holdout"] == holdout]
        by_holdout[holdout] = {
            "n_rows": len(holdout_rows),
            "n_candidates": sum(1 for row in holdout_rows if row["decision"] == "candidate"),
            "n_positive_centered_delta": sum(1 for row in holdout_rows if row["centered_delta_vs_shuffle"] > 0.0),
            "max_bidirectional_recording_fraction": max(
                (row["bidirectional_recording_fraction"] for row in holdout_rows),
                default=0.0,
            ),
        }
    return {
        "n_rows": len(rows),
        "n_regions": len({row["region"] for row in rows}),
        ...
        "decision": "extreme_quantile_region_candidate" if candidates else "no_extreme_quantile_region_candidate",
    }
...
def render_markdown(report: dict) -> str:
    ...
    lines = [
        ...
        "",
        (
            "Strict single-parent-region scan for the response-latency extreme target. "
            "Rows use the same true-vs-shuffle, total-baseline, target-class, and "
            "same-recording bidirectionality gate as the family candidate."
        ),
        "",
    ...
    lines += [
        ...
        (
            ...
            else "At least one parent region passes the strict local gate; validate across shuffle seeds before GPU training."
        ),
        "",
    ]
    return "\n".join(lines)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-md",
        type=Path,
        default=REPO_ROOT / "docs/extreme_quantile_region_specificity.md",
    )
...
def main() -> int:
    ...
    regions = build_region_vocab(vocab["recs"], selected_rids, args.region_granularity)
    ...
    for holdout in holdouts:
        ...
        for region_index, region in enumerate(regions):
            rows.append(evaluate_region_row(
