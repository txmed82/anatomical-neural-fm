...
def render_markdown(
    ...
    if fixed_broad_family_train_arm_runpod_panel is not None:
        summary = fixed_broad_family_train_arm_runpod_panel["summary"]
        lines += [
        ...
        for row in fixed_broad_family_train_arm_runpod_panel["rows"]:
            lines.append(
                f"| {row['holdout']} | {row['target_mode']} | "
                f"{row['true_centered_auc']:.4f} | {row['shuffle_centered_auc']:.4f} | "
                f"{row['centered_delta_vs_shuffle']:+.4f} |"
        ...
        lines += [
            "",
            (
                "Decision: the fixed broad-family count arm now has a bounded cloud "
    ...
    if correct_low_contrast_choice_family_gate is not None:
        summary = correct_low_contrast_choice_family_gate["summary"]
        balances = summary["target_balances"]
        top = summary["top_rows"][:6]
        lines += [
        ...
        lines += ["", "Decision: the current panel has no correct low-contrast choice training trigger.", ""]
    if correct_low_contrast_choice_projected_gate is not None:
        summary = correct_low_contrast_choice_projected_gate["summary"]
        balances = summary["target_balances"]
        top = summary["top_rows"][:6]
        lines += [
