"""Audit block-prior-aligned choice targets under the shared-family gate."""
...
def target_name_for_threshold(threshold: float) -> str:
    return f"prior_aligned_choice_le_{threshold:g}"


def per_recording_prior_aligned_choice_labels(rec, *, max_contrast: float) -> np.ndarray:
    ...
    prior_side = np.where(probability_left > 0.5, -1.0, np.where(probability_left < 0.5, 1.0, 0.0))
    valid = (
        np.isfinite(stim_on)
        & np.isfinite(choice)
        & (choice != 0.0)
    ...
    labels[valid & (choice == -prior_side)] = 0.0
    labels[valid & (choice == prior_side)] = 1.0
    return labels


def build_prior_aligned_choice_trial_samples(
    ...
    *,
    ...
    for rid in rids:
        rec = recs[rid]
        labels = per_recording_prior_aligned_choice_labels(rec, max_contrast=max_contrast)
        stim_on = _finite_array(rec.trials.stim_on_times)
...
def precompute_prior_threshold(**kwargs) -> dict:
    ...
    low_contrast.build_low_contrast_choice_trial_samples = build_prior_aligned_choice_trial_samples
...
def summarize(rows: list[dict], balances: dict[str, dict]) -> dict:
    ...
    base["decision"] = (
        "prior_aligned_choice_family_candidate"
        if base["n_candidates"]
        else "no_prior_aligned_choice_family_candidate"
    )
    base["next_action"] = (
        "Validate prior-aligned choice candidates across shuffle seeds before GPU training."
        if base["n_candidates"]
        else "Do not train: prior-aligned choice targets do not pass the strict local gate."
    )
    return base
...
def render_markdown(report: dict) -> str:
    ...
    text = text.replace("# Low-Contrast Choice Family Gate", "# Prior-Aligned Choice Family Gate", 1)
    text = text.replace(
        "restrict trials by absolute stimulus contrast and classify left-vs-right choice",
        "restrict trials by absolute stimulus contrast and classify whether choice aligns with the biased block prior",
        1,
    )
    return text
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-json", type=Path, default=REPO_ROOT / "docs/prior_aligned_choice_family_gate.json")
    parser.add_argument("--out-md", type=Path, default=REPO_ROOT / "docs/prior_aligned_choice_family_gate.md")
...
def main() -> int:
    ...
    for threshold in args.max_contrast:
        payload = precompute_prior_threshold(
            ds=ds,
            vocab=vocab,
