#!/bin/bash
# CSH_ZAD_019 anatomy-control check.
#
# Compares true region labels against a shuffled-label control while keeping the
# same held-out subject, target, model size, and seeds.
set -euo pipefail
...
OUT_ROOT="${OUT_ROOT:-runs/lso_csh_zad_019_region_shuffle}"
MANIFEST="${MANIFEST:-}"
...
for seed in $SEEDS; do
  for arm in shared_baseline region_only region_shuffle; do
    ...
    mkdir -p "$out"
    echo ""
    echo "=== holdout=CSH_ZAD_019 arm=$arm seed=$seed ==="
