...
This is a positive control result for the current candidate signal. True
`region_only` beats the shared null by +0.042 AUC on average and is positive in
3/3 seeds. The shuffled-region control preserves the marginal region-label
distribution but breaks the unit-to-region mapping; it drops to -0.012 mean
delta. That argues the `CSH_ZAD_019` lift is tied to anatomical region identity,
not merely extra parameters or label-frequency structure.

This is not yet a broad cross-animal claim because the effect is still
subject-specific. The next confirmation should test `CSH_ZAD_019` under
shared-region masking or parent-region granularity.
